...
class WebApplicationGenerator {
	...
	def run(File path, String profile, CockpitConfiguration cockpitConfiguration) throws Exception {
		LOGGER.info("Running the Generator.")
		if (cockpitConfiguration !== null) {
			LOGGER.info("Found files and loading entries.")
			generate(path, profile, cockpitConfiguration)
	...
	private def generate(File path, String profile, CockpitConfiguration cockpitConfiguration) {
